...
import { GetModularOptionListProps, renderWithHelpers } from '../../testUtil';
...
const initHookTest = (props: GetModularOptionListProps) => {
  // const selectProps = getModularOptionListProps(props) as GetModularOptionListProps;
  const helpers = renderWithHelpers(props);
  return {
    ...helpers,
...
describe('useElementDetection', () => {
  const createLinkedData = (helpers: ReturnType<typeof initHookTest>): LinkedData[] => {
    // list of all known elements, theirs getters and checkers
    const linkedData: LinkedData[] = [
      { type: 'list', getElement: helpers.getListElement, isChecker: isListType },
      { type: 'button', getElement: helpers.getButtonElement, isChecker: isSelectedOptionsButtonType },
  ...
  // some elements are only rendered if multiSelect = true
  const multiSelectOnlyElementTypes: KnownElementType[] = ['showAllButton', 'clearAllButton', 'tagList'];

  // some elements are only rendered if options are selected
  ...
  // some elements are only rendered with groups
  const requiresGroupsElementTypes: KnownElementType[] = ['listGroupLabel'];
  // some elements are only rendered if list is open
  const requiresOpenListElementTypes: KnownElementType[] = ['listItem', 'listGroupLabel'];
  // some elements are only rendered if there is an input
  ...
  // filter possible elements by current <ModularOptionList> props
  const filterLinkedData = (
    data: LinkedData[],
    { groups, multiSelect, input, open, hasSelections }: GetModularOptionListProps,
  ) => {
  ...
  const defaultProps: GetModularOptionListProps = {
    groups: true,
    multiSelect: true,
    input: 'filter',
    hasSelections: true,
  ...
  describe('getElementByKnownType', () => {
    it('returns element by type or null. Other element selectors should not return same elements.', async () => {
      ...
      linkedData.forEach((data) => {
        ...
        // cross check with other selectors.
