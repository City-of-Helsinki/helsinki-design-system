...
export const DateInput = React.forwardRef<HTMLInputElement, DateInputProps>(
  (
    ...
    const dateFormatSeparator = dateFormat.split('').find((char) => !/[a-zA-Z0-9]/.test(char));
    ...
    const handleInputChange = (value: string) => {
      // then use the separator to handle disallowed characters (allow only numbers and the separator)
      const disallowedCharacters = new RegExp(`[^0-9${dateFormatSeparator}]+`);
