...
export const defaultTexts: Record<SupportedLanguage, Texts> = {
  en: {
    assistive: '',
    clearButtonAriaLabel_one: 'Clear search "{{value}}".',
    clearButtonAriaLabel_multiple: 'Clear search.',
    error: '',
    label: '',
    ...
    searchErrorTitle: "We couldn't load the search results.",
    ...
    assistive: '',
    clearButtonAriaLabel_one: 'Tyhjennä haku "{{value}}".',
    clearButtonAriaLabel_multiple: 'Tyhjennä haku.',
    error: '',
    label: '',
  ...
  sv: {
    assistive: '',
    clearButtonAriaLabel_one: 'Rensa sökning "{{value}}".',
    clearButtonAriaLabel_multiple: 'Rensa sökning.',
    error: '',
    label: '',
    noSelectedOptions: '0 valda alternativ',
    placeholder: 'Sök efter alternativ',
    ...
    multiSelectGroupAriaLabel: '{{label}} (välj alla)',
    ...
    searchErrorTitle: 'Vi kunde inte ladda sökresultaten.',
...
const createTextInterpolationContent = (metaData: SearchMetaData): TextInterpolationContent => {
...
export const createTextProvider = (texts: SearchProps['texts']): TextProvider => {
  return typeof texts === 'function' ? texts : textObjToTextProvider(texts || {});
};

export const appendTexts = (texts: Partial<SearchProps['texts']>, metaData: SearchMetaData) => {
...
export const getTextFromMetaData = (key: TextKey, metaData: SearchMetaData): string | undefined => {
  return getTextKey(key, metaData);
};

export const getTextFromDataHandlers = (key: TextKey, dataHandlers: SearchDataHandlers): string | undefined => {
