...
export interface ScreenReaderConfig {
  /**
   * CSS class name for the notifications container
   */
  className: string;
  /**
   * Test ID for the notifications container
   */
  testId: string;
  /**
   * Data handlers with getMetaData and updateMetaData functions
   */
  dataHandlers: {
    getMetaData: () => { screenReaderNotifications: ScreenReaderNotification[] };
    updateMetaData: (data: { screenReaderNotifications: ScreenReaderNotification[] }) => void;
  };
}

/**
 * Generic ScreenReaderNotifications hook that manages screen reader announcements
 */
export function useGenericScreenReaderNotifications(config: ScreenReaderConfig) {
  const { getMetaData, updateMetaData } = config.dataHandlers;
  ...
  }, [forceRender, getCurrentNotifications]);
...
/**
 * Generic ScreenReaderNotifications component that can be configured for different dropdown components
 */
export function GenericScreenReaderNotifications({ config }: { config: ScreenReaderConfig }) {
  const { notifications, timeNow } = useGenericScreenReaderNotifications(config);

  return (
    <div aria-live="polite" className={config.className} data-testid={config.testId}>
      {notifications.map((n) => {
        if (!n.showTime) {
