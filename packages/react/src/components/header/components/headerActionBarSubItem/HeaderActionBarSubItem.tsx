import React, { cloneElement, HTMLAttributes, PropsWithChildren, ReactNode, useCallback } from 'react';
...
export interface HeaderActionBarSubItemProps extends ElementProps {
  /**
   * The ref is forwarded to the root anchor or button element.
   */
...
export const HeaderActionBarSubItem = ({
  ...
  if (linkAttr.onClick) {
    return (
      <Wrapper>
        <button type="button" className={itemClassName} ref={ref as React.Ref<HTMLButtonElement>} {...linkAttr}>
          <Content />
        </button>
      </Wrapper>
    );
