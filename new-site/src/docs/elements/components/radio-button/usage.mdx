...
export const DefaultRadioButtonExample = () => {
  ...
  return (
    <>
      <RadioButton id="radio3" name="example" label="Label" value="1" checked={selectedItem2 === "1"} onChange={onChange2}/>
      <RadioButton id="radio4" name="example" label="Label" value="2" checked={selectedItem2 === "2"} onChange={onChange2} />
    </>
  );
};
