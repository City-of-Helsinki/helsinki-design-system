...
title: 'NumberInput - Usage'
---

import { NumberInput, Link } from 'hds-react';
...
- HDS Number input also supports displaying an unit for the number. Displaying the unit is not mandatory. Only use it when you think it will help the user.
    - Display the unit for <Link size="M" external href="https://en.wikipedia.org/wiki/International_System_of_Units">SI units</Link> (such as meters) and currencies.
