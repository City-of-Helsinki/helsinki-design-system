...
export const ToggleExample = () => {
  ...
  return (
    <ToggleButton id="toggle-button" label="Label" checked={checked1} onChange={() => setChecked1(!checked1)} />
  );
};
...
export const HorizontalToggleExample = () => {
  ...
  return (
    <div style={{ maxWidth: '160px' }}>
      <ToggleButton id="toggle-button-horizontal" label="Label" variant="inline" checked={checked2} onChange={() => setChecked2(!checked2)} />
    </div>
  );
};
