...
export const ToggleExample = () => {
  ...
  return <ToggleButton id="toggle-button" label="Label" checked={checked1} onChange={() => setChecked1(!checked1)} />;
};
...
- The toggle button should have an immediate effect when activated.
  ...
  - If your toggle button does not have an immediate effect, use a single <InternalLink href="/components/checkbox">Checkbox component</InternalLink> instead. Also see <InternalLink href="/foundation/guidelines/checkbox-radiobutton-toggle">guidelines for choosing between radiobuttons, checkboxes and toggles.</InternalLink>
...
export const DefaultToggleExample = () => {
  ...
  return (
    <ToggleButton id="toggle-button-default" label="Label" checked={checked2} onChange={() => setChecked2(!checked2)} />
  );
};
