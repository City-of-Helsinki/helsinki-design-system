...
<Playground scope={{ SelectionGroup, Checkbox, useState }}>

```jsx
import { SelectionGroup, Checkbox } from 'hds-react';

() => {
  const [checkedItems, setCheckedItems] = useState({ "v-checkbox1": true, "v-checkbox2": true });
  const onChange = (event) => {
    setCheckedItems({ ...checkedItems, [event.target.name]: event.target.checked });
  };
  return (
    <SelectionGroup label="Vertical checkbox group" helperText="Assistive text">
...
<fieldset class="hds-selection-group">
  ...
  <div class="hds-selection-group__description-text-gap description-text">Description text</div>
  ...
  <div class="hds-selection-group__helper-text-gap helper-text">Assistive text</div>
</fieldset>
```
...
<Playground scope={{ SelectionGroup, RadioButton, useState }}>
...
() => {
  const [checkedItems, setCheckedItems] = useState({ "h-checkbox1": true, "h-checkbox2": true });
  const onChange = (event) => {
    setCheckedItems({ ...checkedItems, [event.target.name]: event.target.checked });
  };
  return (
    <SelectionGroup label="Horizontal checkbox group" direction="horizontal" helperText="Assistive text">
...
| ----------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **HDS React**     | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabAriaLabel="Opens in a new tab" href="/storybook/react/?path=/story/components-selectiongroup--default">View in Storybook</Link> | <ExternalLink openInNewTab openInNewTabAriaLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/tree/master/packages/react/src/components/selectionGroup">View source</ExternalLink> |
| **HDS Core**      | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabAriaLabel="Opens in a new tab" href="/storybook/core/?path=/story/components-selection-group--default">View in Storybook</Link> | <ExternalLink openInNewTab openInNewTabAriaLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/tree/master/packages/core/src/components/selection-group">View source</ExternalLink> |

### Properties
Note! You can find the full list of properties in the <Link openInNewTab openInNewTabAriaLabel="Opens in a new tab" href="/storybook/react/?path=/docs/components-selectiongroup--default">React Storybook.</Link>
...
| `descriptionText` | The description text content that will be shown above the selection group content.    | `string`                    | -             |
