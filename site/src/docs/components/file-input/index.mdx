...
export const FileInputDefaultValue = () => {
  try {
    const [files, setFiles] = React.useState([
      typeof window !== undefined ? new window.File(['string content'], 'dummy.txt', { type: 'text/plain' }) : null,
      typeof window !== undefined ? new window.File(['string content with more text'], 'anotherDummy.txt', { type: 'text/plain' }) : null,
    ]);
    console.log('Selected files: ', files);
