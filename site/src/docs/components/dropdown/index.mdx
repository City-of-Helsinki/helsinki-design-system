...
- Dropdowns usually have four (4) or more options. When having only 2-3 options, it is usually better to use <InternalLink href="/components/radio-button">radio buttons</InternalLink> or <InternalLink href="/components/checkbox">checkboxes</InternalLink>. Also, if options need to be easily comparable (for example, product pricing), prefer radio buttons over dropdown.
