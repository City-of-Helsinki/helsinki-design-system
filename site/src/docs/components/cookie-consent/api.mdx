...
Common texts, groups and cookie data can be found in <ExternalLink openInExternalDomainAriaLabel="Opens in a new domain" openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/blob/master/packages/react/src/components/cookieConsentCore/example/minimal_sitesettings.json">minimal sitesettings example</ExternalLink>. The file contains all user interface strings, language options, descriptions for common cookie groups, and details about common Helsinki cookies.
There is also an example <ExternalLink openInExternalDomainAriaLabel="Opens in a new domain" openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/blob/master/packages/react/src/components/cookieConsentCore/example/common_sitesettings.json">hel.fi sitesettings example</ExternalLink> with all the translations. The file contains all user interface strings, language options, descriptions for common cookie groups, and details about common Helsinki cookies. You can also override parts of it and add your project-specific groups and cookies. Remember to replace the `INSERT_YOUR_DOMAIN_NAME_HERE.hel.fi` -strings in the settings file with the domain name of your service.
...
| ---------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `focusTargetSelector`        | No       | CSS selector for the element that receives focus when the banner is closed without reloading the page, helping keyboard users return to the control that opened it.                                                                      |
| `language`                   | No       | Language used for the banner and cookie settings. If a requested translation is unavailable, the configured fallback language is used; if that is also unavailable, the first available translation is used. Defaults to `en`.          |
| `theme`                     | No       | Visual theme applied to the banner and cookie settings. Defaults to `bus`.                                                                                                                                                                |
| `pageContentSelector`        | No       | Selector for the page content element. The component adds bottom scroll margin to elements inside this element so content is not hidden behind the fixed banner. Defaults to `body`.                                                   |
| `settingsPageSelector`       | No       | If this selector matches an element on the page, the component renders the full-page cookie settings there instead of rendering the banner. The matched element's contents are replaced. Not used by the React context or `useCookieConsent` hook. Example: `#hds-cookie-consent-full-page`. |
| `spacerParentSelector`       | No       | Selector for the element that contains the banner spacer. The spacer reserves room for the banner when it is fixed at the bottom of the viewport. Defaults to `body`.                                                                     |
| `submitEvent`                | No       | If `true`, dispatches a window-level `hds-cookie-consent-changed` event with `detail.acceptedGroups` when consent changes instead of reloading the page. Defaults to `false`, but is always `true` in the React context and hooks. |
| `targetSelector`             | No       | Selector for the element where the banner is rendered. Full-page settings use `settingsPageSelector` instead. Defaults to `body`.                                                                                                         |
| `disableAutoRender`          | No       | If `true`, prevents the component from rendering the banner or settings page automatically. Use the instance methods to render them manually. Always `true` in the React context and `useCookieConsent` hook. Defaults to `false`. |
...
| Property                           | Required | Description                                                                                                                                   |
| ---------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `cookieName`                       | Yes      | Name of the cookie used to store the user's consent choices. Use a distinct name for each service whose consent settings must be kept separate.                                |
| `fallbackLanguage`                 | No       | Language used when a requested translation is unavailable. If the translation is also unavailable in this language, the first available translation is used.                                                                            |
| `groupsWhitelistedForApi`          | No       | Groups that may be accepted programmatically with `setGroupsStatusToAccepted(['group-id'])`. Only groups listed here can be accepted through that method.                         |
| `languages`                        | Yes      | Languages supported by the banner and settings page, including each language's code, display name, and text direction.                                                          |
| `monitorInterval`                  | No       | Interval in milliseconds for checking cookies and other browser storage for items without consent. `0` disables monitoring; positive values below 500 are treated as 500.       |
| `optionalGroups`                   | No       | Cookie groups that the user can accept or reject, together with the group descriptions and stored items shown in the settings.                                                   |
| `remove`                           | No       | If `true`, removes cookies and other stored items that the monitor finds without the user's consent. If `false`, those items are reported but left in storage. Defaults to `false`. |
| `robotCookies`                     | No       | Browser storage items that are allowed without being shown to the user. Use this for items set by automated tools, such as Siteimprove, that need to work while testing the site. |
| `requiredGroups`                   | Yes      | Cookie groups that are necessary for the service to work. They are always treated as accepted and are shown to the user with their descriptions and stored items.                 |
| `siteName`                         | No       | Name of the service. Used to identify the settings and can be inserted into banner-level translated text with the `{{siteName}}` placeholder. The placeholder is not supported in group or cookie translations.                       |
