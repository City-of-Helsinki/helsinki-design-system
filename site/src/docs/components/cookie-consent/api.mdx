...
| `monitorInterval`                  | No       | Monitoring rate in milliseconds. Values greater than zero enable monitoring; intervals are clamped to a minimum of 500 ms.                    |
| `optionalGroups`                   | No       | List of optional cookie groups and their descriptions. Defaults to an empty array when omitted.                                               |
...
| `removeBanner`                                           | Removes the banner and its DOM, styles, timers, and observers.                                                                                                                                                                                     | `(setFocus?: boolean) => void`                                          |
| `removePage`                                             | Removes the rendered consents. Returns `true` if there was a page to remove.                                                                                                                                                                       | `() => boolean`                                                         |
| `renderPage`                                             | Renders the consents to the element matching the supplied selector, or the configured `settingsPageSelector`.                                                                                                                                      | `(settingsPageSelector?: string) => void`                               |
| `dispose`                                                | Disposes the instance's monitoring and DOM resources.                                                                                                                                                                                              | `() => void`                                                            |
...
Consents that the user has given are saved to the configured `cookieName` cookie.

Cookies are always strings. The cookie value is JSON containing consent groups, checksums, timestamps, and, when needed, a `showBanner` flag. For example:
...
// the banner opens automatically when needed
```

Note that the `CookieConsentCore.create` is an asynchronous function.

It is recommended to set `options.submitEvent` to `true` when using the <InternalLink href="/components/cookie-consent/#rendered-on-a-page">page</InternalLink> version. Otherwise the whole page is reloaded after save and the user will not see a notification about successful save or receive a change event.

The whole page is also reloaded when using the <InternalLink href="/components/cookie-consent/#banner">banner</InternalLink> without `options.submitEvent`, but the banner is closed when the consents are stored and before the page reloads.
...
`useCookieSettingsPage` accepts an optional `settingsPageSelector` property.
When provided, the settings are rendered into the matching element; otherwise
the default element identified by `settingsPageId` is used.
...
`siteSettings` and initialization options are read when the core instance is
created. They are not reloaded automatically when their object identity changes
after mount. Language and theme can be changed through the returned instance;
other option changes require disposing and creating a new instance.
...
`settingsPageId` identifies the default element where the React settings page
is rendered. The standalone `CookieConsentCore` API uses
`settingsPageSelector` instead. The `renderPage` function accepts an optional
CSS selector when rendering to a specific element.
...
| `renderPage`                                          | Renders the consents to the element matching the configured settings page selector, or to an optionally supplied CSS selector.                                                                                                                     | `(selector?: string) => Promise<boolean>`                             | -                              |
| `removePage`                                          | Removes the rendered consents.                                                                                                                                                                                                                     | `()=>void`                                                            | -                              |
| `settingsPageId`                                      | The ID of the default settings-page element. Used by the `useCookieSettingsPage` and `CookieSettingsPage`.                                                                                                                                         | `string`                                                              | `hds-cookie-consent-full-page` |
