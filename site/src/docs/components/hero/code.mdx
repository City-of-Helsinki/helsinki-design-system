---
slug: '/components/hero/code'
title: 'Hero - Code'
---

import { Button, Hero } from 'hds-react';
import TabsLayout from './tabs.mdx';
import NativeElementPropsInfo from '../../../components/NativeElementPropsInfo';

export default ({ children, pageContext }) => <TabsLayout pageContext={pageContext}>{children}</TabsLayout>;

## Code

### Code examples

#### Diagonal koros

Note that in order to customise the colour of the background with `variant="diagonalKoros"`, you need to set the `--koros-color` CSS variable.

<Playground scope={{ Hero, Button }}>

```jsx
import { Fieldset, TextInput } from 'hds-react';

() => (
  <Hero
    imageSrc="https://hds.hel.fi/images/homepage/amos58.jpg"
    variant="diagonalKoros"
    theme={{
      '--color': '#000',
      '--koros-color': '#f5a3c7',
    }}
  >
    <Hero.Title>Hero title</Hero.Title>
    <Hero.Text>
      This is an introduction text. The length is circa two sentences. The suitable total length of the title and
      introduction is around 230 characters, including spaces. The intro ends with a period.
    </Hero.Text>
    <Button
      variant="secondary"
      role="link"
      style={{ '--background-color': '#000', '--color': '#fff', '--border-color': '#000' }}
    >
      Call to action
    </Button>
  </Hero>
);
```

```html
<style scoped>
  .custom-theme {
    --koros-color: #f5a3c7;
    --color: #000;
    --koros-height: 34px;
  }
  .black-button.black-button {
    --background-color: #000;
    --color: #fff;
    --border-color: #000;
  }
</style>
<div>
  <div class="hds-hero custom-theme hds-hero--diagonal-koros">
    <div class="hds-hero--with-background__container">
      <div class="hds-hero__content">
        <div class="hds-hero--with-background__content-columns">
          <div class="hds-hero__card">
            <h1 class="hds-hero__title">Hero title</h1>
            <p class="hds-hero__text">
              This is an introduction text. The length is circa two sentences. The suitable total length of the title
              and introduction is around 230 characters, including spaces. The intro ends with a period.
            </p>
            <button
              type="button"
              class="hds-button hds-button--secondary hds-button--theme-black black-button"
              role="link"
            >
              <span>Call to action</span>
            </button>
          </div>
          <div class="hds-hero--with-background__empty-column"></div>
        </div>
      </div>
      <div class="hds-hero--diagonal-koros__koros-aligner">
        <div class="hds-hero--diagonal-koros__koros-and-background">
          <div class="hds-hero__koros-container">
            <div class="hds-koros" style="fill: var(--koros-color);">
              <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="100%" height="85">
                <defs>
                  <pattern id="koros_basic-5" x="0" y="0" width="106" height="85" patternUnits="userSpaceOnUse">
                    <path transform="scale(5.3)" d="M0,800h20V0c-4.9,0-5,2.6-9.9,2.6S5,0,0,0V800z"></path>
                  </pattern>
                </defs>
                <rect fill="url(#koros_basic-5)" width="100%" height="85"></rect>
              </svg>
            </div>
          </div>
        </div>
      </div>
      <div class="hds-hero--with-background__background">
        <img class="hds-hero__image" src="https://hds.hel.fi/images/homepage/amos58.jpg" aria-hidden="true" alt="" />
      </div>
    </div>
  </div>
</div>
```

</Playground>

#### Diagonal koros with arrow

Hero has an option to show an arrow icon to indicate the followin content. The arrow color can also be customised with the `theme` prop by passing `--arrow-icon-color` in it.

<Playground scope={{ Hero, Button }}>

```jsx
import { Fieldset, TextInput } from 'hds-react';

() => (
  <Hero
    imageSrc="https://hds.hel.fi/images/homepage/amos58.jpg"
    variant="diagonalKoros"
    theme={{
      '--color': '#000',
      '--koros-color': '#f5a3c7',
    }}
    showArrowIcon
  >
    <Hero.Title>Hero title</Hero.Title>
    <Hero.Text>
      This is an introduction text. The length is circa two sentences. The suitable total length of the title and
      introduction is around 230 characters, including spaces. The intro ends with a period.
    </Hero.Text>
    <Button
      variant="secondary"
      role="link"
      style={{ '--background-color': '#000', '--color': '#fff', '--border-color': '#000' }}
    >
      Call to action
    </Button>
  </Hero>
);
```

```html
<style scoped>
  .custom-theme {
    --koros-color: #f5a3c7;
    --color: #000;
    --koros-height: 34px;
  }
  .black-button.black-button {
    --background-color: #000;
    --color: #fff;
    --border-color: #000;
  }
</style>
<div>
  <div class="hds-hero custom-theme hds-hero--diagonal-koros hds-hero__arrow-icon-spacing-after">
    <div class="hds-hero--with-background__container">
      <div class="hds-hero__content">
        <div class="hds-hero--with-background__content-columns">
          <div class="hds-hero__card">
            <h1 class="hds-hero__title">Hero title</h1>
            <p class="hds-hero__text">
              This is an introduction text. The length is circa two sentences. The suitable total length of the title
              and introduction is around 230 characters, including spaces. The intro ends with a period.
            </p>
            <button
              type="button"
              class="hds-button hds-button--secondary hds-button--theme-black black-button"
              role="link"
            >
              <span>Call to action</span>
            </button>
          </div>
          <div class="hds-hero--with-background__empty-column"></div>
        </div>
      </div>
      <div class="hds-hero--diagonal-koros__koros-aligner">
        <div class="hds-hero--diagonal-koros__koros-and-background">
          <div class="hds-hero__koros-container">
            <div class="hds-koros" style="fill: var(--koros-color);">
              <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="100%" height="85">
                <defs>
                  <pattern id="koros_basic-5" x="0" y="0" width="106" height="85" patternUnits="userSpaceOnUse">
                    <path transform="scale(5.3)" d="M0,800h20V0c-4.9,0-5,2.6-9.9,2.6S5,0,0,0V800z"></path>
                  </pattern>
                </defs>
                <rect fill="url(#koros_basic-5)" width="100%" height="85"></rect>
              </svg>
            </div>
          </div>
        </div>
      </div>
      <div class="hds-hero--with-background__background">
        <img class="hds-hero__image" src="https://hds.hel.fi/images/homepage/amos58.jpg" aria-hidden="true" alt="" />
      </div>
    </div>
    <div class="hds-hero__arrow-icon-container">
      <span class="hds-icon hds-hero--arrow-icon hds-icon--arrow-down" />
    </div>
  </div>
</div>
```

</Playground>

**Note:** Hero with diagonal Koros adjusts its contents based on browser width. Preview here might might not scale correcly. <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/react/?path=/story/components-hero--diagonal-koros">View Hero with diagonal Koros example in Storybook</Link>.

#### Diagonal koros with additional information

Hero has an option to show additional information below the koros. Usually the information is photographer credit.

<Playground scope={{ Hero, Button }}>

```jsx
import { Fieldset, TextInput } from 'hds-react';

() => (
  <Hero
    imageSrc="https://hds.hel.fi/images/homepage/amos58.jpg"
    variant="diagonalKoros"
    theme={{
      '--color': '#000',
      '--koros-color': '#f5a3c7',
    }}
    information="Photo: Firstname Middlename Long-Lastname"
  >
    <Hero.Title>Hero title</Hero.Title>
    <Hero.Text>
      This is an introduction text. The length is circa two sentences. The suitable total length of the title and
      introduction is around 230 characters, including spaces. The intro ends with a period.
    </Hero.Text>
    <Button
      variant="secondary"
      role="link"
      style={{ '--background-color': '#000', '--color': '#fff', '--border-color': '#000' }}
    >
      Call to action
    </Button>
  </Hero>
);
```

```html
<style scoped>
  .custom-theme {
    --koros-color: #f5a3c7;
    --color: #000;
    --koros-height: 34px;
  }
  .black-button.black-button {
    --background-color: #000;
    --color: #fff;
    --border-color: #000;
  }
</style>
<div>
  <div class="hds-hero custom-theme hds-hero--diagonal-koros hds-hero__arrow-icon-spacing-after">
    <div class="hds-hero--with-background__container">
      <div class="hds-hero__content">
        <div class="hds-hero--with-background__content-columns">
          <div class="hds-hero__card">
            <h1 class="hds-hero__title">Hero title</h1>
            <p class="hds-hero__text">
              This is an introduction text. The length is circa two sentences. The suitable total length of the title
              and introduction is around 230 characters, including spaces. The intro ends with a period.
            </p>
            <button
              type="button"
              class="hds-button hds-button--secondary hds-button--theme-black black-button"
              role="link"
            >
              <span class="hds-button__label">Call to action</span>
            </button>
          </div>
          <div class="hds-hero--with-background__empty-column"></div>
        </div>
      </div>
      <div class="hds-hero--diagonal-koros__koros-aligner">
        <div class="hds-hero--diagonal-koros__koros-and-background">
          <div class="hds-hero__koros-container">
            <div class="hds-koros" style="fill: var(--koros-color);">
              <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" width="100%" height="85">
                <defs>
                  <pattern id="koros_basic-5" x="0" y="0" width="106" height="85" patternUnits="userSpaceOnUse">
                    <path transform="scale(5.3)" d="M0,800h20V0c-4.9,0-5,2.6-9.9,2.6S5,0,0,0V800z"></path>
                  </pattern>
                </defs>
                <rect fill="url(#koros_basic-5)" width="100%" height="85"></rect>
              </svg>
            </div>
          </div>
        </div>
      </div>
      <div class="hds-hero--with-background__background">
        <img class="hds-hero__image" src="https://hds.hel.fi/images/homepage/amos58.jpg" aria-hidden="true" alt="" />
      </div>
    </div>
    <div class="hds-hero__bottom-container" aria-hidden="true">
      <div class="hds-hero__bottom-content-aligner">
        <div class="hds-hero__information-container">
          <span>Photo: Firstname Middlename Long-Lastname</span>
        </div>
      </div>
    </div>
  </div>
</div>
```

</Playground>

**Note:** Hero with diagonal Koros adjusts its contents based on browser width. Preview here might might not scale correcly. <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/react/?path=/story/components-hero--diagonal-koros">View Hero with diagonal Koros example in Storybook</Link>.

### Packages

| Package       | Included                                                                                        | Storybook link                                                                                                                                                    | Source link                                                                                                                                                                                                                |
| ------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **HDS React** | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/react/?path=?path=/story/components-hero--diagonal-koros">View in Storybook</Link> | <ExternalLink openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/blob/master/packages/react/src/components/hero/Hero.tsx">View source</ExternalLink> |
| **HDS Core**  | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/core/?path=/story/components-hero--diagonal-koros">View in Storybook</Link>        | <ExternalLink openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/tree/master/packages/core/src/components/hero">View source</ExternalLink>           |

### Properties

| Property                  | Description                                                                                                          | Values                                                                       | Default value                                                        |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `theme`                   | Theme prop for customisation of the Hero component.                                                                  | `object`                                                                     | <Link href="/components/hero/customisation">See customisation</Link> |
| `koros`                   | Koros prop for customisation of the Koros of the Hero component.                                                     | `object`                                                                     | <Link href="/components/koros/customisation">See koros</Link>        |
| `imageSrc`                | Url of the image                                                                                                     | `string`                                                                     | -                                                                    |
| `centerContent`           | Should the text be aligned to center                                                                                 | `boolean`                                                                    | false                                                                |
| `variant`                 | Which variant to render. The default value is `noImage` if `imageSrc` is not set. Otherwise default is `imageLeft`   | `imageLeft, imageRight, backgroundImage, imageBottom,diagonalKoros, noImage` | `noImage` or `imageLeft`                                             |
| `showArrowIcon`           | Adds the arrow icon in the bottom left corner, slightly overlapping the Hero                                         | `boolean`                                                                    | false                                                                |
| `information`             | Additional information seen below the hero. Usually photographer credits. The element is hidden from screen readers. | `string`                                                                     | -                                                                    |
| [Table 1:Hero properties] |

<NativeElementPropsInfo nodeName="div" />{' '}

| Property                        | Description                 | Values   | Default value |
| ------------------------------- | --------------------------- | -------- | ------------- |
| `headingLevel`                  | Heading level of the title. | `number` | 1             |
| [Table 2:Hero.Title properties] |

<NativeElementPropsInfo nodeName="h1" />{' '}

| Property                       | Description                    | Values | Default value |
| ------------------------------ | ------------------------------ | ------ | ------------- |
| -                              | Only native element properties | -      | -             |
| [Table 3:Hero.Text properties] |

<NativeElementPropsInfo nodeName="p" />{' '}
