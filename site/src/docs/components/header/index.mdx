...
- Please note that under the S breakpoint (576px) the ActionBarItems and ActionBarButtons don't fit in the screen well and hiding them should be considered. We are working on a solution for this.
...
<PlaygroundPreview>
  ...
  >
    ...
    >
      ...
      <Header.ActionBarButton label="Kirjaudu" icon={<IconSignin />} onClick={() => { window.alert("Kirjaudu clicked"); }} fixedRightPosition />
    </Header.ActionBar>
    <Header.NavigationMenu>
      <Header.Link
        href="#"
