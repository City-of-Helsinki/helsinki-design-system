---
slug: '/components/table/accessibility'
...
- A horizontally scrollable table container is focusable, so that it can also be scrolled with a keyboard.
  - The shadows shown at the edges of a scrollable container are decorative. They are rendered as pseudo elements, so they are not exposed to assistive technology, and they are never the only indication of scrollable content, because in the Core version they are not shown in all browsers.
