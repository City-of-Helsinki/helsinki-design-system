...
- Table container is allowed to be scrollable. This is common on smaller screens where it can be impossible to fit the full table on the device screen.
  ...
  - When the table can be scrolled horizontally, a shadow is shown at that edge of the container to indicate that there is more content in that direction. The shadow disappears when the end is reached. See [Horizontal scroll shadows](#horizontal-scroll-shadows) variant example below.
...
The default HDS table variant includes horizontal lines. The header row can be either dark or light depending on the brand colour used as a background. Whether you need a black or white text colour can be checked from <InternalLink href="/foundation/design-tokens/colour/accessibility#brand-colour-combinations">HDS Colour documentation page</InternalLink>
...
When the table is wider than its container, a shadow is shown at the edge of the container to indicate that there is more content in that direction. The shadow disappears when the table is scrolled to that end. Scroll the example table below to see the shadows appear and disappear.

The shadows are decorative. They are not the only indication of scrollable content, and they are not exposed to assistive technology.

The shadows need no configuration in either version. In the React version they are shown in every browser. In the Core version they are shown with CSS only, which means that browsers without support for CSS scroll-driven animations show the table without shadows.

export const ScrollShadowsExample = () => {
  const cols = [
    { key: 'id', headerName: 'Not rendered' },
    { key: 'firstName', headerName: 'First name' },
    { key: 'surname', headerName: 'Surname' },
  ...
  const caption = (
    <span>
      <b>Table 9</b>: Service users (horizontally scrollable)
    </span>
  );
  return (
    <div style={{ maxWidth: '500px' }}>
...
<PlaygroundPreview>
  <ScrollShadowsExample />
</PlaygroundPreview>

#### Without the outer border

Tables have an outer border by default. It can be left out when the table is placed inside an element that already has a border of its own. The lines inside the table are not affected.

export const WithoutOuterBorderExample = () => {
  const cols = [
    { key: 'id', headerName: 'Not rendered' },
    { key: 'firstName', headerName: 'First name' },
    { key: 'surname', headerName: 'Surname' },
  ...
  const caption = (
    <span>
      <b>Table 10</b>: Service users (without the outer border)
    </span>
  );
  return (
    <div style={{ maxWidth: '640px' }}>
