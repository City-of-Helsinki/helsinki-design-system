...
<PlaygroundPreview>
  <Link href="https://hds.hel.fi/" size="M" style={{ display: 'block', width: 'fit-content' }}>Link to HDS</Link>
</PlaygroundPreview>
...
Note! These icons are meant to describe the link target. If you need a directional icon for the link (e.g. an arrow), you should instead use (<InternalLink href="/components/buttons#supplementary">a supplementary button</InternalLink> with `role="link"`.
