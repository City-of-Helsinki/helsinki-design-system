...
- Remember to tell users if the link is going to open in a new tab. In HDS React Link this feature is provided through prop `openInNewTab`. The link shows visible text (e.g. "(avautuu uudessa välilehdessä)") at the end of the link, or inside trailing parentheses when the link text already ends with parenthetical text. This can be customized with the `openInNewTabLabel` prop. The new-tab text is visible only and is not added to the aria-label.
  In HDS Core, set a proper `aria-label` to the anchor tag as shown in the examples below.
