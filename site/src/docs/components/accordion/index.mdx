...
If you have multiple accordions one below another or many accordions in the same view, consider using the visually more light [default styling variant](#default) instead of this one. If Accordions in a Card with borders and stacked is what is wanted, use the same sized variant for all and remove the top borders after the 1st Accordion as in <InternalLink href="/components/accordion/code#stacked-accordions">stacked Accordions example</InternalLink>.
