...
import { Button, Notification, NotificationSize, TextInput } from 'hds-react';
...
Use `type="error"`, `autofocus`, and `notificationAriaLabel="Error summary"` to use Notification as an error summary for form validation. The `autofocus` prop moves keyboard and screen reader focus directly to the notification label when it mounts. Pass a submission counter as `key` so that re-submitting a form with errors remounts the notification and re-triggers focus.

<Playground scope={{ Notification, TextInput, Button, useState }}>


```jsx
import { Notification, TextInput, Button } from 'hds-react';
...
() => {
  ...
  const errorList = [
    errors.firstName && { id: 'cd-first-name', message: errors.firstName },
    errors.lastName && { id: 'cd-last-name', message: errors.lastName },
    errors.email && { id: 'cd-email', message: errors.email },
  ].filter(Boolean);

  return (
    <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-s)' }}>
      {errorList.length > 0 && (
        <Notification
          key={submitCount}
          type="error"
...
| ----------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **HDS React**     | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/react/?path=/story/components-notification--default">View in Storybook</Link> | <ExternalLink openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/tree/master/packages/react/src/components/notification">View source</ExternalLink> |
| **HDS Core**      | <div style={{ display: 'flex', gap: 'var(--spacing-3-xs)' }}><IconCheckCircleFill /> Yes </div> | <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/core/?path=/story/components-notification--default">View in Storybook</Link>  | <ExternalLink openInNewTab openInNewTabLabel="Opens in a new tab" href="https://github.com/City-of-Helsinki/helsinki-design-system/tree/master/packages/core/src/components/notification">View source</ExternalLink>  |

### Properties
Note! You can find the full list of properties in the <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/storybook/react/?path=/story/components-notification--default">React Storybook.</Link>

| Property                   | Description                                                                                                                 | Values                                                                                                        | Default value    |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ---------------- |
| `autofocus`                | If set to true, focuses the notification label after it mounts. Use when displaying a form error summary so that keyboard and screen reader users are moved directly to the error list. | `boolean`                                                                                       | false            |
| `autoClose`                | If set to true, the Toast notification will be closed automatically after a certain time.                                   | `boolean`                                                                                                     | false            |
| `autoCloseDuration`        | If set, closes the notification after given milliseconds. If unset, calculates the milliseconds according to textual content length (with a minimum of 4000ms). | `number`                                                                  | -                |
...
| `size`                     | The size of the notification.                                                                                               | `NotificationSize` or (`"small"`, `"medium"`, `"large"`)                                                      | `"medium"`       |
| `link`                     | Link below the notification content. Always renders as `medium` size.                                                       | <Link openInNewTab openInNewTabLabel="Opens in a new tab" href="/components/link/">`Link`</Link>          | -                |
