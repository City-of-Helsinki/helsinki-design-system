...
Components are updated to use the new token instead of hardcoded value. While this change was released already in an earlier version, updating to 3.0.0 is a good opprotunity to make this change. 

### Media query hooks
There are hooks <code>useMediaQueryLessThan</code> and <code>useMediaQueryGreaterThan</code> to ease the use of media queries in React. While this change was released already in an earlier version, updating to 3.0.0 is a good opprotunity to make this change.
...
It will be available with Helsinki licenses and in the future as a community file.

- <ExternalLink size="M" openInNewTab="true">HDS Figma library in City of Helsinki organization</ExternalLink>

## For developers
There are also some updates on the dependencies. Make sure to dependencies get updated with running command <code>yarn</code>.