...
<Notification label=";tldr; Check at least these" className="siteNotification">
  When updating to HDS 4.0.0 check and test at least these: - dropdown(Combobox and select) is replaced with whole new
  select - cookieConsent is replaced with new CookieConsent and now also available in core and standalone js - separate
  ariaLabel is replaced with native aria-label - iconLeft is now iconStart and iconRight is now iconEnd - size variants
  in components are changed to enums - Use of Sketch ends, check Figma files instead
</Notification>
...
Notification has its `size` prop changed to enum.
Notification size is now `NotificationSize.Small`, `NotificationSize.Medium` or `NotificationSize.Large`. The medium value is the default.
...
All texts are passed as an object or returned from a function.
Removed props include:
...
The `onChange`receives now 3 arguments:
