...
- `clearable` (always clearable). Can be prevented with the onChange callback.
- `theme` (only one theme, className can be used for overriding css vars).
...
```jsx
<Select texts={{ label: 'Label', placeholder: 'Placeholder' }} />
```
