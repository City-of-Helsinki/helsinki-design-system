...
<Notification label=";tldr; Check at least these" className="siteNotification">
  When updating to HDS 4.0.0 check and test at least these:
  - dropdown(Combobox and select) is replaced with whole new select
  - cookieConsent is replaced with new CookieConsent and now also available in core and standalone js
  - separate ariaLabel is replaced with native aria-label
  - iconLeft is now iconStart and iconRight is now iconEnd
  - size variants in components are changed to enums
  - Use of Sketch ends, check Figma files instead
</Notification>
