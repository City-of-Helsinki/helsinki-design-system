...
- **Content should also be provided at least in Swedish (SW) and English (EN), if possible.**
...
- **The preferred method for switching languages is with the language picker action in [HDS Navigation component](https://hds.hel.fi/components/navigation "HDS Navigation component documentation").**
  - The available languages can also be listed as links in the navigation bar in stead of dropdown, if the services navigation is simple and there are less than three available languages.
