...
- If possible, do input validation in client-side real-time and provide the user with immediate feedback. See [HDS Form validation pattern page](/patterns/form-validation) for more information and best practices with password fields.
...
- In City of Helsinki Services, always use Helsinki Profiili authentication if possible. In practice, this will often mean that the service itself will not need to use the password field or any login form of its own, because account logins will forward to the Helsinki Profiili login screen.
