...
- Multiple simultaneous loading spinners can be very confusing for assistive technologies. **HDS Loading spinner React component is built in a way that only reads one alert for screen readers** - even if there were multiple on the screen. If you are not able to use this feature, it is recommended to only show one loading spinner at a time.
