...
When applicable, an unit can be set for the input by using the `unit` prop.
...
Steppers can be enabled via providing a `step` property. Use this variant when value changes are small and you can determine logical amounts for increase and decrease steps.
