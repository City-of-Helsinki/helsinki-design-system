...
- HDS Tables support sorting. When a table is set to be sortable via the `sortingEnabled` property, all columns of the table will get a sorting icon next to their header.
  - Sorting can be disabled for each column via the `disableSorting` column attribute. This is useful when the column data is not suitable for sorting.
...
### Sorting
The table can be configured to be sortable which makes the sorting button appear after each column header. The sorting icon is different depending on the data type of the column (alphabetical or numeric/other).

Currently, only one column can be sorted at a time. Sorting can be disabled for a column by using the `disableSorting` column attribute.
