...
- One FileInput can be configured to accept one or multiple files.
  - Make sure labels of the input clearly indicate the amount of expected files to the user.
...
- Do not trust the front-end solution to validate the files for you. Validate files on the backend.
  ...
  - HDS FileInput verified the file type only from the file extension name. This validation is not succifient in the backend since it can be bypassed by renaming the file.
- Always restrict and only allow file types related to the use case.
- Pay attention to error messages returned from the backend. Do not include information the the user does not need, e.g. file paths.
- If files are uploaded from untrusted sources, make sure files are stored outside of the web root, with limited permissions, preferably with strong validation.
  - Files from untrusted sources should also be scanned by antivirus scanners to prevent upload of known malicious content.

## Usage and variations

### File input
In its default form, the HDS FileInput offers a button-like element which opens a file browser native to the user's current device. Selected files are presented in a list below the input and files can be removed from the listing separately.
...
If in some cases having the ability to drag and drop files onto the page could help the user, this can be enabled with the `dragAndDrop` property. Note that the traditional file input is still included below the drag and drop area. Therefore this property does not weaken accessibility of the component.
