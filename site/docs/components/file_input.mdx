...
- Do not trust the front-end solution to validate the files for you. Validate files on the backend.
  ...
  - HDS FileInput verified the file type only from the file extension name. This validation is not sufficient in the backend since it can be bypassed by renaming the file.
