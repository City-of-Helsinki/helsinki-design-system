...
- Checkboxes should not have any immediate effects. The checkbox selection takes effect when the user presses a submit button (e.g. in a form). If you need the selection to have an immediate effect, use [HDS Toggle button component](/components/toggle) instead. Also see [guidelines for choosing between radiobuttons, checkboxes and toggles.](/guidelines/checkboxs-radiobuttons-toggles)
