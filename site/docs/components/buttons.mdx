...
import { Button, IconShare, IconAngleRight, IconTrash, Notification, StatusLabel } from "hds-react";
...
### Loading buttons
<p>If an action triggered by the button press is not immediate, a loading button state should be shown to the user to indicate the loading period.</p>
...
**Note!** It is not recommended to use Supplementary buttons for loading actions since they can be hard to the user to differentiate from the loading state button.
...
<Playground>
  {() => {
    ...
    return (
      <>
        <div style={{fontSize: 'var(--fontsize-body-m)', marginBottom: 'var(--spacing-m)'}}>Click the button to see
          loading button state in action. When loading is completed, a toast notification is shown to indicate the
          success.
        </div>
        <Button
          isLoading={isLoading}
...
#### Core code example:
_Feature not supported in hds-core!_
