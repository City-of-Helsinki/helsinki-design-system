...
### Focus colour
HDS components use a custom focus colour styling instead of the default browser provided style. The default HDS focus style is accessible on light backgrounds. Since HDS does not offer dark mode, when using HDS components on darker backgrounds, it may be required to customize the focus style colour. The WCAG requirement for user interface element contrast is `3 : 1`.
...
It is allowed to customize HDS focus style colour to better fit the theme of the service. However, the implementor is always responsible for ensuring that the following WCAG requirements are met:
